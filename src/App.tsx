      ...
      */}
      <header className="sticky top-0 z-50 w-full border-b border-primary-100 bg-paper/90 backdrop-blur-md">
        <div className="container mx-auto flex h-20 items-center justify-between px-6 lg:px-12">
          ...
          <Link
            to="/"
            className="flex items-center gap-2 font-heading font-bold text-3xl text-ink tracking-tight"
          >
            <span>
          ...
          <div className="hidden lg:flex items-center gap-6">
            {user ? (
              <div className="flex items-center gap-5">
                <Link
                  to="/dashboard"
                  className="flex items-center gap-3 text-sm font-medium text-ink/80 hover:text-primary-700 transition-colors"
                >
                  {user.photoURL ? (
                    <img
                      src={user.photoURL}
                      alt={user.displayName || "User"}
                      className="w-9 h-9 rounded-full border border-primary-200 object-cover paper-shadow"
        ...
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden border-t border-primary-100 bg-paper px-6 py-8 space-y-6 shadow-2xl absolute w-full left-0 max-h-[calc(100vh-80px)] overflow-y-auto"
          >
            {navItems.map((item) => {
              const isActive =
                location.pathname === item.path ||
            ...
            <div className="pt-6 border-t border-primary-100/50">
              {user ? (
                <div className="space-y-5">
                  <Link
                    to="/dashboard"
                    className="flex items-center gap-4 text-base font-medium text-ink"
                    onClick={() => setIsOpen(false)}
                  >
                    {user.photoURL ? (
                      <img
                  ...
                  <Button
                    variant="ghost"
                    className="w-full justify-start text-accent-500 hover:bg-accent-50/50 p-0 h-auto"
                    onClick={() => {
                      setIsOpen(false);
                      logout();
                    }}
