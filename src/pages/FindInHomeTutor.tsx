    ...
    <div className="flex flex-col lg:flex-row gap-8 px-4 pb-20 mt-4">
        ...
        <div className="rounded-2xl bg-[#FDFBF7] p-6 sm:p-8 border border-zinc-200 shadow-sm">
          <div className="border-b-2 border-zinc-200 pb-4 mb-6">
            <h2 className="font-extrabold text-zinc-900 uppercase tracking-wider text-base sm:text-lg flex items-center gap-2">
              <FontAwesomeIcon icon={["fas", "filter"]} className="text-xl" />{" "}
              Bộ lọc tìm kiếm
            </h2>
          </div>
          ...
          <div className="space-y-6">
            <div>
              <label className="text-sm font-bold text-zinc-600 mb-2 block uppercase">
                Môn học
              </label>
            ...
            <div>
              ...
              <select className="w-full rounded-xl border border-zinc-200 p-3 text-sm outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-200 bg-zinc-50/50 text-zinc-900 font-medium transition-all cursor-pointer">
                ...
                {HANOI_WARDS.map((ward) => (
                  <option key={ward} value={ward} className="text-zinc-900">
                    {ward}
                  </option>
                ))}
              </select>
      ...
      <div className="flex-1 space-y-8">
        <div className="mb-10 bg-zinc-50 p-6 sm:p-8 rounded-2xl border border-zinc-200 relative overflow-hidden">
          ...
          <p className="text-base sm:text-lg text-zinc-600 font-medium relative z-10 max-w-2xl">
            Đội ngũ gia sư chất lượng cao, sẵn sàng đến tận nhà giảng dạy trong
            khu vực của bạn.
          </p>
        </div>
        ...
        <div className="space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <h2 className="text-lg sm:text-xl font-extrabold text-zinc-900 tracking-tight">
              Tìm thấy{" "}
              <span className="text-zinc-900">{MOCK_TUTORS.length}</span> gia sư
              phù hợp
            </h2>
            <div className="text-sm font-bold text-zinc-500 bg-zinc-100 px-4 py-2 rounded-full self-start sm:self-auto">
                ...
                >
                  ...
                  <div className="shrink-0 relative self-center sm:self-start">
                    <img
                      src={tutor.image}
                      alt={tutor.name}
                      className="h-32 w-32 sm:h-40 sm:w-40 rounded-xl object-cover border-4 border-white shadow-sm"
                    />
                    {tutor.verified && (
                      <div
                        className="absolute -bottom-3 -right-3 bg-zinc-1000 text-white w-10 h-10 rounded-full border-4 border-white flex items-center justify-center shadow-sm"
                        title="Đã xác thực"
                      >
                        ✓
                      </div>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
                        <div className="text-center sm:text-left">
                          ...
                          <span
                            className={`inline-block px-4 py-1.5 rounded-xl text-sm font-bold border ${subjectColor}`}
                          >
                            {tutor.subject}
                          </span>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-6 justify-center sm:justify-start">
                        {tutor.tags.map((tag) => (
                          <span
                            key={tag}
                            className="inline-flex items-center rounded-xl bg-zinc-50 px-3 py-1.5 text-xs font-bold text-zinc-900 border border-zinc-200"
                          >
                            #{tag}
                          </span>
                        ))}
                    ...
                    <div className="flex sm:flex-row flex-col justify-end gap-4 mt-auto pt-6 border-t border-zinc-200">
                      <Button
                        asChild
                        variant="outline"
                        className="w-full sm:w-auto rounded-xl font-bold text-base border-zinc-200 text-zinc-700 hover:bg-zinc-50 hover:text-zinc-900 py-6 px-8 transition-colors"
                      >
