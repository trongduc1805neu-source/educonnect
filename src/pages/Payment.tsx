...
export function Payment() {
  ...
  const paymentState = location.state as { type: 'tutor' | 'class' | 'wallet', data: any } || null;
  
  const [customAmount, setCustomAmount] = useState('500000');

  const title = paymentState?.type === 'tutor' 
  ...
  const fee = paymentState?.type === 'tutor'
  ...
  const amountToNumber = typeof fee === 'string' ? parseInt(fee.replace(/\D/g, '')) : 0;
  
  // Format info string for QR if needed
  const transferMessage = paymentState?.type === 'tutor' 
      ...
      <div className="bg-[#FDFBF7] rounded-[40px] p-8 md:p-12 border border-primary-100 paper-shadow">
        <div className="text-center mb-10">
          ...
          <p className="text-ink/60 font-light max-w-md mx-auto">
            Vui lòng hoàn thành thanh toán để bắt đầu lộ trình học tập của bạn cùng EduConnect.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
            <div className="bg-primary-50/50 rounded-2xl p-6 border border-primary-100 h-full">
              <h3 className="font-heading text-xl text-ink mb-4">Thông tin đăng ký</h3>
              <div className="space-y-4">
                <div>
                  <span className="text-ink/50 text-[11px] uppercase tracking-widest font-bold block mb-1">Nội dung</span>
          ...
          <div className="space-y-6">
            <div className="bg-[#FDFBF7] rounded-2xl p-6 border border-primary-100 text-center flex flex-col items-center justify-center h-full">
              ...
              <div className="text-[13px] text-ink/80 text-left w-full space-y-2 bg-primary-50/30 p-4 rounded-xl">
                <p className="flex justify-between"><span className="text-ink/50">Ngân hàng:</span> <strong>MB Bank</strong></p>
                <p className="flex justify-between"><span className="text-ink/50">Số TK:</span> <strong>0886861805</strong></p>
                <p className="flex justify-between"><span className="text-ink/50">Chủ TK:</span> <strong>TRONG DUC</strong></p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-lg mx-auto mt-10">
          <motion.div whileHover={{ y: -2 }} className="group cursor-pointer">
            <Button onClick={() => navigate('/dashboard')} className="w-full h-14 rounded-2xl text-[13px] tracking-widest font-bold uppercase shadow-lg shadow-primary-700/20">
