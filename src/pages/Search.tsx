...
export function Search() {
  ...
  const [learningMode, setLearningMode] = useState(
    searchParams.get("mode") || "Tất cả hình thức",
  );
  ...
  const filteredTutors = MOCK_TUTORS.filter((tutor) => {
    ...
    if (
      appliedSearchTerm &&
      !tutor.name.toLowerCase().includes(appliedSearchTerm.toLowerCase()) &&
      !tutor.subject.toLowerCase().includes(appliedSearchTerm.toLowerCase())
    ) {
      return false;
    }
    // learning mode
    if (
      appliedLearningMode !== "Tất cả hình thức" &&
      !tutor.mode.toLowerCase().includes(appliedLearningMode.toLowerCase())
    ) {
      return false;
    }
    // subject
    ...
    if (
      appliedArea &&
      !tutor.location.toLowerCase().includes(appliedArea.toLowerCase())
    ) {
      return false;
    }
    // grade
    ...
    <div className="max-w-7xl mx-auto px-6 pb-24 mt-8">
      ...
      <header className="mb-16 max-w-2xl">
        <h1 className="text-4xl lg:text-5xl font-heading text-ink mb-4 leading-tight">
          Danh bạ{" "}
          <span className="italic text-primary-700">người dẫn đường.</span>
        </h1>
        <p className="text-ink/60 text-lg font-light">
          Khám phá những hồ sơ được tinh tuyển kỹ lưỡng, phù hợp với nhịp độ và
          mục tiêu học tập của riêng bạn.
        </p>
      </header>
          ...
          <div className="sticky top-28 bg-[#FDFBF7] rounded-[32px] p-6 lg:p-8 border border-primary-100 shadow-xl shadow-primary-900/5 space-y-6 lg:space-y-8">
            <div className="border-b border-primary-100 pb-4 mb-2">
              <h4 className="text-[12px] font-bold text-ink uppercase tracking-[0.2em] flex items-center gap-2">
                <SearchIcon className="w-4 h-4 text-primary-700" /> Tinh chỉnh
                kết quả
              </h4>
            </div>
              ...
              <div className="pt-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (learningMode !== "Tất cả hình thức")
                      newParams.set("mode", learningMode);
                    if (subject) newParams.set("subject", subject);
                    if (area && learningMode !== "Online")
        ...
        <div className="flex-1 space-y-12">
          {filteredTutors.map((tutor, idx) => (
            ...
            >
              ...
              <Link
                to={`/profile/${tutor.id}`}
                className="w-full md:w-48 shrink-0 block"
              >
              ...
              <div className="flex-1 flex flex-col">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-accent-500 font-semibold text-sm uppercase tracking-widest">
                      {tutor.subject}
                    </p>
                  </div>
                  <div className="text-left md:text-right">
                    <div className="text-xl font-heading text-primary-700 mb-1">
                      {tutor.fee}
                      <span className="text-sm font-sans text-ink/40">/h</span>
                    </div>
                    <span className="text-[11px] font-medium text-ink/40 uppercase tracking-wider">
                ...
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mb-6 text-[13px] text-ink/70">
                  <div className="flex items-center gap-1.5">
                    <MapPin className="w-4 h-4 text-ink/40" /> {tutor.distance}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <BookOpen className="w-4 h-4 text-ink/40" /> {tutor.mode}
                  </div>
                  {tutor.isVerified && (
                ...
                <div className="space-y-2 text-[13px] text-ink/80 font-light leading-relaxed mb-6">
                  <p>
                    <strong className="font-semibold text-ink/50 uppercase text-[10px] tracking-widest mr-2">
                      Học vấn
                    </strong>{" "}
                    {tutor.education}
                  </p>
                ...
                <div className="mt-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 pt-6 border-t border-primary-50">
                  <div className="flex flex-wrap gap-2">
                    {tutor.tags.map((tag) => (
                      <span
                        key={tag}
                        className="inline-block px-3 py-1 bg-primary-50 text-[10px] font-semibold text-primary-900 uppercase tracking-widest rounded-sm"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    <Link
                      to={`/profile/${tutor.id}`}
                      className="text-[11px] font-bold text-ink/50 hover:text-primary-700 uppercase tracking-widest editorial-link"
                    >
                      Xem chi tiết
                    </Link>
                    <Button
                      onClick={() =>
                        navigate("/payment", {
                          state: { type: "tutor", data: tutor },
                        })
                      }
                      className="text-[11px] h-9 px-5 tracking-widest"
                    >
                      KẾT NỐI NGAY
                    </Button>
                  </div>
