...
const MOCK_MATCHING_CLASSES = [
  {
    ...
    description: 'Nhóm chúng mình hiện có 2 thành viên với mục tiêu nắm chắc nền tảng Toán cấp 3. Cần tìm thêm 1-2 bạn đồng hành để cùng nhau giải đề và tạo động lực học tập mỗi tuần.',
  ...
  },
  {
    ...
    description: 'Mình đang kẹt ở band 6.0 và cần tìm những người bạn cùng chí hướng để luyện Speaking và trao đổi ideas. Có giáo viên hướng dẫn trực tiếp sửa lỗi.',
  ...
  },
  {
    ...
    description: 'Lớp luyện thi đại học chuyên sâu do thầy Hoàng trực tiếp đứng lớp. Không gian học tập yên tĩnh, tài liệu được biên soạn riêng. Hiện còn 2 vị trí trống.',
    author: 'Gia sư Hoàng',
    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-24 px-6 mt-8">
      ...
      <header className="mb-16 max-w-3xl">
        ...
        <p className="text-lg text-ink/70 font-light leading-relaxed">
          Không ai phải đi một mình. Kết nối với những tâm hồn đồng điệu để sẻ chia không gian học tập, nhân đôi động lực và lan tỏa tri thức.
        </p>
      </header>
      ...
      <div className="flex gap-8 border-b border-primary-200 pb-px">
        <button 
          ...
          className={`pb-4 text-sm font-bold uppercase tracking-widest transition-all relative ${
            activeTab === 'find' ? 'text-primary-700' : 'text-ink/40 hover:text-ink/70'
          }`}
        >
          Bảng tin lớp ghép
          {activeTab === 'find' && (
            <motion.div layoutId="match-tab" className="absolute bottom-0 left-0 right-0 h-[2px] bg-primary-700" />
          )}
        </button>
        <button 
          onClick={() => setActiveTab('create')}
          className={`pb-4 text-sm font-bold uppercase tracking-widest transition-all relative ${
            activeTab === 'create' ? 'text-primary-700' : 'text-ink/40 hover:text-ink/70'
          }`}
        >
          Tạo lời mời mới
          {activeTab === 'create' && (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {MOCK_MATCHING_CLASSES.map((item, idx) => (
                ...
                >
                  ...
                  <h3 className="text-2xl font-heading text-ink mb-2 group-hover:text-primary-700 transition-colors">{item.subject}</h3>
                  <p className="text-[11px] font-bold text-ink/40 uppercase tracking-widest mb-6">Đăng bởi: {item.author}</p>
                  ...
                  <div className="space-y-4 pt-6 border-t border-primary-50">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-2 text-[13px] text-ink/60">
                        ...
                        <span>Sĩ số: {item.currentStudents}/{item.maxStudents}</span>
                      </div>
                      <div className="text-right">
                        <div className="text-lg font-heading text-primary-700">{item.tuitionPerStudent}</div>
          ...
          >
            ...
            <div className="bg-white p-10 lg:p-14 border border-primary-100 rounded-3xl paper-shadow">
              <div className="flex items-center gap-3 mb-10 pb-6 border-b border-primary-100">
                ...
                <div>
                  <h2 className="text-2xl font-heading text-ink">Mở lời mời mới</h2>
                  <p className="text-sm text-ink/50 font-light mt-1">Chi tiết và chân thành sẽ giúp bạn tìm được người đồng hành phù hợp.</p>
                </div>
              </div>
